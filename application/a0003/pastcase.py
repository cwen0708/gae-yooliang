...
class List(AdministratorHandler):
    def get(self, *args):
        ...
        self.page_now = page
        self.page_all = self.sql.pager("SELECT count(1) FROM PastCase", (), size)
        self.results = self.sql.query_all("SELECT * FROM PastCase ORDER BY sort DESC LIMIT %s, %s", ((page - 1) * size, size), (page - 1) * size)
...
class Create(AdministratorHandler):
    ...
    def post(self, *args):
        ...
        self.json({"info": u'歷屆案例已新增', "content": u"您已經成功的新增了一筆歷屆案例。"})
...
class Edit(AdministratorHandler):
    def get(self, *args):
        ...
        if id != u"":
            self.record = self.sql.query_one("SELECT * FROM PastCase where id = %s", id)
    ...
    def post(self, *args):
        ...
        self.json({"info": u'歷屆案例已更新', "content": u"您已經成功的變更了此筆歷屆案例。"})
