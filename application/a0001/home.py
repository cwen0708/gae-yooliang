...
class index(ProductMenuHandler):
    def get(self, *args):
        ...
        self.tvprogram_list = self.sql.query_all('SELECT * FROM TVProgram WHERE is_enable = 1 AND is_delete = 0 and end_time > %s ORDER BY start_time, sort DESC LIMIT %s, %s', (now, 0, 2))
