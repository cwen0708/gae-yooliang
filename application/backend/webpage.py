...
class list(AdministratorHandler):
    def get(self, *args):
        ...
        self.page_now = page
        self.page_all = self.sql.pager('SELECT count(1) FROM WebPage', (), size)
        self.results = self.sql.query_all('SELECT * FROM WebPage ORDER BY sort DESC LIMIT %s, %s', ((page - 1) * size, size))
        for item in self.results:
            item["title"] = item["setting_name"]
        self.render("/v1/webpage/list.html", "backend")


class create(AdministratorHandler):
    def get(self, *args):
        self.render("/v1/webpage/create.html", "backend")
    ...
    def post(self, *args):
        ...
        if self.sql.query_one('SELECT * FROM WebPage where setting_no = %s', setting_no) is not None:
            self.json({"info": u'網站頁面新增失敗', "content": u"此編號已有被使用，請換一個。"})
            return

        self.sql.insert("WebPage", {
...
class edit(AdministratorHandler):
    def get(self, *args):
        ...
        if id != '':
            self.record = self.sql.query_one('SELECT * FROM WebPage where id = %s', id)
        self.render("/v1/webpage/edit.html", "backend")
    ...
    def post(self, *args):
        ...
        if self.sql.query_one('SELECT * FROM WebPage where setting_no = %s and id != %s', (setting_no, id)) is not None:
            self.json({"info": u'網站頁面更新失敗', "content": u"此編號已有被使用，請換一個。"})
            return

        self.sql.update("WebPage", {
            "setting_name": setting_name,
            "setting_no": setting_no,
            "value": value,
        }, {
