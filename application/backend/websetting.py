...
class list(AdministratorHandler):
    def get(self, *args):
        ...
        self.page_now = page
        self.page_all = self.sql.pager('SELECT count(1) FROM WebSetting', (), size)
        self.results = self.sql.query_all('SELECT * FROM WebSetting ORDER BY sort DESC LIMIT %s, %s', ((page - 1) * size, size))
        for item in self.results:
            item["title"] = item["setting_name"]
        self.render("/v1/websetting/list.html", "backend")


class create(AdministratorHandler):
    def get(self, *args):
        self.render("/v1/websetting/create.html", "backend")
    ...
    def post(self, *args):
        ...
        if self.sql.query_one('SELECT * FROM WebSetting where setting_no = %s', setting_no) is not None:
            self.json({"info": u'網站設定新增失敗', "content": u"此編號已有被使用，請換一個。"})
            return

        self.sql.insert("WebSetting", {
...
class edit(AdministratorHandler):
    def get(self, *args):
        ...
        if id != '':
            self.record = self.sql.query_one('SELECT * FROM WebSetting where id = %s', id)
        self.render("/v1/websetting/edit.html", "backend")
    ...
    def post(self, *args):
        ...
        if self.sql.query_one('SELECT * FROM WebSetting where setting_no = %s and id != %s', (setting_no, id)) is not None:
            self.json({"info": u'網站設定更新失敗', "content": u"此編號已有被使用，請換一個。"})
            return

        self.sql.update("WebSetting", {
            "setting_name": setting_name,
            "setting_no": setting_no,
            "value": value,
        }, {
